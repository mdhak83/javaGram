...
import static org.telegram.utils.StreamingUtils.readTLString;
import static org.telegram.utils.StreamingUtils.writeInt;
import static org.telegram.utils.StreamingUtils.writeLong;
import static org.telegram.utils.StreamingUtils.writeTLObject;
import static org.telegram.utils.StreamingUtils.writeTLString;
...
public class TLVector<T> extends TLObject implements List<T> {
    ...
    public void deserializeBody(InputStream stream, TLContext context) throws IOException {
        ...
        for (int i = 0; i < count; i++) {
            Logger.d("TLVECTOR", "reading: " + (i+1) + " from " + count + " (" + this.items.size() + ")" + " --> " + this.destClass);
            if (this.destClass == Integer.class) {
