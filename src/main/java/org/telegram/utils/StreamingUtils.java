...
import org.telegram.api._primitives.TLBoolFalse;
import org.telegram.api._primitives.TLBoolTrue;
...
public class StreamingUtils {
     ...
     *
     * @param stream source stream
     * @param charset charset to use to decode the string
     * @return value
     * @throws IOException an IO exception
     */
    public static String readTLString(InputStream stream, Charset charset) throws IOException {
        return new String(readTLBytes(stream), charset);
    }
    ...
    public static String readTLString(InputStream stream) throws IOException {
        return new String(readTLBytes(stream), UTF8);
    }
    ...
    public static void skipBytes(int count, InputStream stream) throws IOException {
        ...
        stream.read(bytes);
        Logger.d("SKIPPED", "Skipped " + count + " bytes: " + Arrays.toString(bytes));
        //stream.skip(count);
    }
     ...
     * Reading tl-vector from stream
     *
     * @param <T> Object class described by the vector
     * @param stream  source stream
     * @param context tl-context
     * @param vectorType the vector type
     * @return tl-vector the tl-vector
     * @throws IOException an IO exception
     */
    ...
    public static boolean readTLBool(InputStream stream) throws IOException {
        ...
        switch (v) {
            case TLBoolTrue.CLASS_ID:
                return true;
            case TLBoolFalse.CLASS_ID:
                return false;
            default:
    ...
    /**
     * Converts byte array to hexadecimal or decimal string
     *
     * @param array array of bytes to convert
     * @param toHex True if conversion to hexadecimal, decimal otherwise.
     * @param separator String to separate each converted  value
     * @return A sequence of bytes converted to their ASCII hexadecimal or decimal values, separated by a separator.
     */
    public static String bytesToString(byte[] array, boolean toHex, String separator) {
        StringBuilder sb = new StringBuilder();
        for (byte b : array) {
            if (toHex) {
                sb.append(String.format("%02X" + separator, b));
            } else {
                sb.append(String.format("%03d" + separator, b));
            }
        }
        return sb.toString();
    }
    ...
    /**
     * Converts byte array to its hexadecimal ASCII representation
     *
     * @param array array of bytes to convert
     * @return A sequence of bytes converted to their ASCII hexadecimal separated by ":"
     */
    public static String bytesToString(byte[] array) {
        return bytesToString(array, true, ":");
    }
